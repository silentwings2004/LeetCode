...
/**
 * 滑窗的同时，实时维护窗口内的逆序对个数 inv
 * 元素 x 进入窗口时，inv 增加了窗口内的大于 x 的元素个数。
 * 元素 x 离开窗口时，inv 减少了窗口内的小于 x 的元素个数。
 * 这可以用值域树状数组动态维护。维护元素的出现次数的前缀和。
 * 由于元素范围很大，我们又只需要知道元素的相对大小（元素的绝对大小不重要）。
 * 所以可以先离散化，把元素映射到 n 以内，
 */